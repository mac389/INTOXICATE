...
tbl1 <- clean_data %>%
  tbl_cross(`Exposure Category`, Gender, percent = 'column') %>%
  add_overall() %>%
  add_p() 
...
tbl1 %>%
  as_kable_extra(
    format = "latex",
    booktabs = TRUE,
    caption = "\\textbf{No association between gender and type of xenobiotic.}. Cells denote case count (column percentage). F, biological female. M, male. See X for description of exposure categories."
  ) %>%
...
We found no overall association between gender and exposure category (`r p_value_g_by_x`, Table \ref{tab:table1}). 

# Table 2. Focused Comparisons: High-Frequency Categories vs All Others

```{r table2, results='asis', tab.cap = FALSE}

xenobiotics <- c("Antidepressants", "Analgesic", "Street Drugs")

indiv_comparisons <- map_dfr(xenobiotics, function(category) {
  binary_data <- create_binary_comparison(clean_data, category)
  tbl <- binary_data %>% 
    select(category_binary, Gender) %>%
    tbl_cross(label = list(category_binary ~ "Class")) %>%
    add_overall() %>%
    add_p() %>%
    modify_table_body(~.x |> dplyr::mutate(p.value = p.adjust(p.value, method = "bonferroni", n = length(xenobiotics)))) 

  tibble(name = category,
         binary_data = list(binary_data),
         contingency_tbl = list(tbl))
})

for (i in 1:nrow(indiv_comparisons)) {
   cat("\n\n")
   print(
    indiv_comparisons$contingency_tbl[[i]] %>%
      as_kable_extra(
        format = "latex",
        booktabs = TRUE,
        caption = sprintf(
  "%s vs overall counts. Cells denote case count (column percentage).",
  indiv_comparisons$name[i]), 
        label = sprintf("\\label{tab:%s-v-others}", tolower(gsub(" ", "-", indiv_comparisons$name[i])))))
      }
```
